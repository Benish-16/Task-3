...
<html lang="en">
  ...
  $dotenv = Dotenv\Dotenv::createImmutable('C:/Users/91600/Documents/javascript'); 
  $dotenv->load();
...
if(empty($_POST['password'])){
  ...
  if (empty($nameErr) && empty($emailErr) && empty($passwordErr) ) {
    ...
    if ($stmt->execute()) {
      ...
      <strong>Success!</strong> Your details have been successfully Login.
...
$sql = "SELECT * FROM login WHERE email=?";
 ...
 if($result1->num_rows>0){
  ...
  if (password_verify($password1 , $hashedPasswordFromDb)) {
    echo '<div class="alert alert-success alert-dismissible fade show" role="alert" id="alert1">
    <strong>Success!</strong> Your details have been successfully signin.
    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
</div>';
